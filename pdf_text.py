...
@app.route('/')
def home():
    return render_template('extractpdfs.html')
            ...
            with open("data.txt", "r") as f:
                for line in f:
                    if line.strip():
                        ...
                        break
                if header:
                    table_name = [{"Table": header}]
            # Convert the list of dictionaries to JSON
            json_data = json.dumps([table_name,property_list], indent=4)
...
def process_pdf():
    ...
    if found_key == 'RENT ROLL' or found_key == 'Rent Roll':
        ...
        with open("sample1.txt", "r") as f:
            ...
            if header:
                table_name = [{"Table": header}]
        json_response = json.dumps([table_name, list1], indent=4)
    ...
    if found_key == '':
        ...
        respond = keyword_in_image('uploaded_files/', search_keywords)
        return respond
